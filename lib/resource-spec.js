...
function validateSingleResourceSpec(spec, fallbackEntityName, label) {
    ...
    for (let i = 0; i < spec.fields.length; i++) {
        ...
        if (field.type === 'Enum') {
            ...
            field.enumValues = normalizeEnumValues(raw.enumValues ?? raw.values, `${fieldLabel}.enumValues`);
        }
        ...
        if (field.type === 'Enum' && field.enumValues === undefined) {
            field.enumValues = inferEnumValuesFromUi(field.ui);
        }
    ...
    const ui = normalizeUiConfig(spec.ui, `${label}.ui`);
...
function normalizeEnumValues(value, label) {
    ...
    for (let i = 0; i < value.length; i++) {
        ...
        if (typeof item !== 'string' || !item.trim()) {
            throw new Error(`${itemLabel} must be a non-empty string.`);
        }

        const enumValue = item.trim();
        if (!/^[A-Z][A-Z0-9_]*$/.test(enumValue)) {
            throw new Error(`${itemLabel} must be UPPER_SNAKE_CASE. Example: ACTIVE, IN_PROGRESS.`);
        }
...
function inferEnumValuesFromUi(ui) {
    const options = ui?.table?.filter?.options;
...
function normalizeUiConfig(ui, label) {
    ...
    if (ui.create !== undefined && ui.create !== null) {
        result.create = normalizeUiScreen(ui.create, `${label}.create`);
    }

    if (ui.edit !== undefined && ui.edit !== null) {
        result.edit = normalizeUiScreen(ui.edit, `${label}.edit`);
    }

    if (ui.detail !== undefined && ui.detail !== null) {
        result.detail = normalizeUiScreen(ui.detail, `${label}.detail`);
    }

    if (ui.form !== undefined && ui.form !== null) {
        result.form = normalizeUiScreen(ui.form, `${label}.form`);
    }
    ...
    if (ui.table !== undefined && ui.table !== null) {
        ...
        if (ui.table.searchFields !== undefined) {
            table.searchFields = normalizeStringArray(ui.table.searchFields, `${label}.table.searchFields`);
        }

        if (ui.table.defaultColumns !== undefined) {
            table.defaultColumns = normalizeStringArray(ui.table.defaultColumns, `${label}.table.defaultColumns`);
        }
        ...
        if (ui.table.filter !== undefined && ui.table.filter !== null) {
            ...
            if (ui.table.filter.placement !== undefined) {
                if (!['inline', 'toolbar'].includes(ui.table.filter.placement)) {
                    throw new Error(`${label}.table.filter.placement must be "inline" or "toolbar".`);
                }
            ...
            if (ui.table.filter.type !== undefined) {
                if (!['text', 'select', 'multi-select', 'date', 'date-range', 'lookup', 'boolean', 'toggle'].includes(ui.table.filter.type)) {
        ...
        if (ui.table.filters !== undefined) {
            if (!Array.isArray(ui.table.filters)) {
                throw new Error(`${label}.table.filters must be an array.`);
            }

            table.filters = ui.table.filters.map((filterConfig, index) =>
                normalizeTableFilterReference(filterConfig, `${label}.table.filters[${index}]`),
            );
        }
    ...
    if (ui.lookup !== undefined && ui.lookup !== null) {
        if (typeof ui.lookup !== 'object' || Array.isArray(ui.lookup)) {
            throw new Error(`${label}.lookup must be a JSON object.`);
        }

        const lookup = {};

        if (ui.lookup.labelFields !== undefined) {
            lookup.labelFields = normalizeStringArray(ui.lookup.labelFields, `${label}.lookup.labelFields`);
        }

        if (ui.lookup.searchFields !== undefined) {
            lookup.searchFields = normalizeStringArray(ui.lookup.searchFields, `${label}.lookup.searchFields`);
        }

        if (ui.lookup.descriptionFields !== undefined) {
            lookup.descriptionFields = normalizeStringArray(ui.lookup.descriptionFields, `${label}.lookup.descriptionFields`);
        }

        if (ui.lookup.columns !== undefined) {
            if (!Array.isArray(ui.lookup.columns) || ui.lookup.columns.length === 0) {
                throw new Error(`${label}.lookup.columns must be a non-empty array.`);
            }

            lookup.columns = ui.lookup.columns.map((column, index) => {
                const columnLabel = `${label}.lookup.columns[${index}]`;

                if (typeof column === 'string') {
                    const field = column.trim();
                    if (!field) {
                        throw new Error(`${columnLabel} must not be empty.`);
                    }

                    return { field, label: _.startCase(field) };
                }

                if (!column || typeof column !== 'object' || Array.isArray(column)) {
                    throw new Error(`${columnLabel} must be a string or JSON object.`);
                }

                if (typeof column.field !== 'string' || !column.field.trim()) {
                    throw new Error(`${columnLabel}.field must be a non-empty string.`);
                }

                if (column.label !== undefined && typeof column.label !== 'string') {
                    throw new Error(`${columnLabel}.label must be a string.`);
                }

                return {
                    field: column.field.trim(),
                    label: column.label?.trim() || _.startCase(column.field),
                };
            });
        }
...
function normalizeUiScreen(screen, label) {
    ...
    if (screen.tabs !== undefined) {
        if (!Array.isArray(screen.tabs)) {
            throw new Error(`${label}.tabs must be an array.`);
        }

        result.tabs = screen.tabs.map((tab, index) =>
            normalizeUiTab(tab, `${label}.tabs[${index}]`),
        );
    }
    ...
    if (screen.sections !== undefined) {
        ...
        result.sections = screen.sections.map((section, index) =>
            normalizeUiLayoutSection(section, `${label}.sections[${index}]`),
        );
    }

    if (screen.rows !== undefined) {
        if (!Array.isArray(screen.rows)) {
            throw new Error(`${label}.rows must be an array.`);
        }

        result.rows = screen.rows.map((row, index) =>
            normalizeUiLayoutRow(row, `${label}.rows[${index}]`),
        );
    }
...
function normalizeUiTab(tab, label) {
    ...
    result.id = tab.id.trim();

    if (!/^[a-z][a-z0-9-]*$/.test(result.id)) {
        throw new Error(`${label}.id must be kebab-case. Example: general, certification-identification.`);
    }
    ...
    if (tab.description !== undefined) {
        ...
        result.description = tab.description.trim();
    }
    ...
    if (tab.sections !== undefined) {
        ...
        result.sections = tab.sections.map((section, index) =>
            normalizeUiLayoutSection(section, `${label}.sections[${index}]`),
        );
    }
    ...
    if (tab.rows !== undefined) {
        ...
        result.rows = tab.rows.map((row, index) =>
            normalizeUiLayoutRow(row, `${label}.rows[${index}]`),
        );
    }
    ...
    if (tab.children !== undefined) {
        ...
        result.children = tab.children.map((child, index) =>
            normalizeUiTabChild(child, `${label}.children[${index}]`),
        );
    }
...
function normalizeUiTabChild(child, label) {
    if (!child || typeof child !== 'object' || Array.isArray(child)) {
        throw new Error(`${label} must be a JSON object.`);
    }

    if (typeof child.resource !== 'string' || !child.resource.trim()) {
        throw new Error(`${label}.resource must be a non-empty string.`);
    }

    const resource = child.resource.trim();
    const resourceResult = validateEntityName(resource);
    if (resourceResult !== true) {
        throw new Error(`${label}.resource is invalid: ${resourceResult}`);
    }
    ...
    if (child.title !== undefined) {
        ...
        result.title = child.title.trim();
    }

    return result;
}

function normalizeUiLayoutSection(section, label) {
    ...
    if (section.title !== undefined) {
        ...
        result.title = section.title.trim();
    }
    ...
    if (section.description !== undefined) {
        ...
        result.description = section.description.trim();
    }
    ...
    result.rows = section.rows.map((row, index) =>
        normalizeUiLayoutRow(row, `${label}.rows[${index}]`),
    );
...
function normalizeUiLayoutRow(row, label) {
    ...
    return {
        columns: row.columns.map((column, index) =>
            normalizeUiLayoutColumn(column, `${label}.columns[${index}]`),
        ),
    };
}

function normalizeUiLayoutColumn(column, label) {
    if (!column || typeof column !== 'object' || Array.isArray(column)) {
        throw new Error(`${label} must be a JSON object.`);
    }
    ...
    const nameResult = validateFieldName(field);
    if (nameResult !== true) {
        throw new Error(`${label}.field is invalid: ${nameResult}`);
    }
    ...
    if (column.span !== undefined) {
        ...
        if (!Number.isFinite(span) || span <= 0) {
            throw new Error(`${label}.span must be a positive number.`);
        }

        result.span = Math.round(span);
    }

    return result;
}

function normalizeTableFilterReference(filterConfig, label) {
    if (typeof filterConfig === 'string') {
        const field = filterConfig.trim();
        if (!field) {
            throw new Error(`${label} must not be empty.`);
        }

        return { field, placement: 'toolbar' };
    }

    if (!filterConfig || typeof filterConfig !== 'object' || Array.isArray(filterConfig)) {
        throw new Error(`${label} must be a string or JSON object.`);
    }

    if (typeof filterConfig.field !== 'string' || !filterConfig.field.trim()) {
        throw new Error(`${label}.field must be a non-empty string.`);
    }

    const filter = {
        field: filterConfig.field.trim(),
    };

    if (filterConfig.placement !== undefined) {
        if (!['inline', 'toolbar'].includes(filterConfig.placement)) {
            throw new Error(`${label}.placement must be "inline" or "toolbar".`);
        }

        filter.placement = filterConfig.placement;
    } else {
        filter.placement = 'toolbar';
    }

    if (filterConfig.type !== undefined) {
        if (!['text', 'select', 'multi-select', 'date', 'date-range', 'lookup', 'boolean', 'toggle'].includes(filterConfig.type)) {
    ...
    if (filterConfig.options !== undefined) {
        ...
        filter.options = filterConfig.options.map((option, index) => {
