/**
 * api-resource-templates.js
 *
 * Builds the template context (token values) for API resource generation.
 * Actual templates live in templates/api/resource/ as plain files.
 *
 * This module computes the dynamic parts — field declarations, imports,
 * annotations, SQL columns — that get injected into the template tokens.
 */
const _ = require('lodash');
const pluralize = require('pluralize');

const TYPE_MAP = {
    String: { javaType: 'String', javaImports: [], sqlType: (f) => `VARCHAR(${f.length || 255})` },
    Text: { javaType: 'String', javaImports: [], sqlType: () => 'TEXT' },
    MediumText: { javaType: 'String', javaImports: [], sqlType: () => 'MEDIUMTEXT' },
    Integer: { javaType: 'Integer', javaImports: [], sqlType: () => 'INT' },
    Long: { javaType: 'Long', javaImports: [], sqlType: () => 'BIGINT' },
    BigDecimal: { javaType: 'BigDecimal', javaImports: ['java.math.BigDecimal'], sqlType: () => 'DECIMAL(19,4)' },
    Double: { javaType: 'Double', javaImports: [], sqlType: () => 'DOUBLE' },
    Boolean: { javaType: 'Boolean', javaImports: [], sqlType: () => 'BOOLEAN' },
    Date: { javaType: 'LocalDate', javaImports: ['java.time.LocalDate'], sqlType: () => 'DATE' },
    DateTime: { javaType: 'LocalDateTime', javaImports: ['java.time.LocalDateTime'], sqlType: () => 'DATETIME(6)' },
    Instant: { javaType: 'Instant', javaImports: ['java.time.Instant'], sqlType: () => 'TIMESTAMP(6)' },
    Enum: { javaType: null, javaImports: [], sqlType: () => 'TINYINT UNSIGNED' },
    ManyToOne: { javaType: null, javaImports: [], sqlType: () => 'BIGINT' },
};

const STRING_TYPES = new Set(['String', 'Text', 'MediumText']);
const INTEGER_TYPES = new Set(['Integer', 'Long']);
const DECIMAL_TYPES = new Set(['BigDecimal', 'Double']);
const DATE_TYPES = new Set(['Date', 'DateTime', 'Instant']);

// ─── Helpers ───────────────────────────────────────────────────────────

function javaType(field) {
    if (field.type === 'Enum') return field.enumName;
    if (field.type === 'ManyToOne') return field.refEntity;
    return TYPE_MAP[field.type].javaType;
}

function dtoJavaType(field) {
    if (field.type === 'ManyToOne') return 'String';
    if (field.type === 'Enum') return field.enumName;
    return TYPE_MAP[field.type].javaType;
}

function collectImports(fields, pkg) {
    const imports = new Set();
    for (const f of fields) {
        if (pkg && f.type === 'Enum') {
            imports.add(`${pkg}.domain.model.${f.enumName}`);
        }

        const mapping = TYPE_MAP[f.type];
        if (mapping) {
            mapping.javaImports.forEach((i) => imports.add(i));
        }
    }
    return [...imports].sort();
}

function formatImports(importList) {
    if (!importList.length) return '';
    return importList.map((i) => `import ${i};`).join('\n') + '\n';
}

function isIncluded(field, dtoName) {
    return !field.dto || field.dto[dtoName] !== false;
}

function fieldsForDto(fields, dtoName) {
    return fields.filter((f) => isIncluded(f, dtoName));
}

function escapeJavaString(value) {
    return String(value)
        .replace(/\\/g, '\\\\')
        .replace(/"/g, '\\"');
}

function buildTableName(pluginPrefix, entityName) {
    const baseTableName = pluralize(_.snakeCase(entityName));
    return pluginPrefix ? `${pluginPrefix}_${baseTableName}` : baseTableName;
}

// ─── Field Renderers ───────────────────────────────────────────────────

function renderDomainModelFields(fields) {
    return fields.map((f) => {
        if (f.type === 'ManyToOne') {
            return `    private ${f.refEntity} ${f.name};`;
        }
        return `    private ${javaType(f)} ${f.name};`;
    }).join('\n');
}

function renderDtoFields(fields) {
    return fields.map((f) => {
        const annotations = renderDtoValidationAnnotations(f);
        const annotStr = annotations.length ? annotations.join('\n') + '\n' : '';
        const fieldName = f.type === 'ManyToOne' ? f.name + 'Id' : f.name;
        return `${annotStr}    private ${dtoJavaType(f)} ${fieldName};`;
    }).join('\n\n');
}

function renderDtoValidationAnnotations(field) {
    const annotations = [];
    const validation = field.validation || {};

    if (field.required) {
        if (STRING_TYPES.has(field.type)) {
            annotations.push('    @NotBlank');
        } else {
            annotations.push('    @NotNull');
        }
    }

    if (STRING_TYPES.has(field.type)) {
        const sizeParts = [];
        if (validation.minLength !== undefined) sizeParts.push(`min = ${validation.minLength}`);

        const maxLength = validation.maxLength !== undefined
            ? validation.maxLength
            : field.type === 'String' && field.length
                ? field.length
                : undefined;

        if (maxLength !== undefined) sizeParts.push(`max = ${maxLength}`);
        if (sizeParts.length) annotations.push(`    @Size(${sizeParts.join(', ')})`);

        if (validation.email) annotations.push('    @Email');
        if (validation.pattern) {
            const attrs = [`regexp = "${escapeJavaString(validation.pattern)}"`];
            if (validation.patternMessage) attrs.push(`message = "${escapeJavaString(validation.patternMessage)}"`);
            annotations.push(`    @Pattern(${attrs.join(', ')})`);
        }
    }

    if (INTEGER_TYPES.has(field.type)) {
        if (validation.min !== undefined) annotations.push(`    @Min(${validation.min})`);
        if (validation.max !== undefined) annotations.push(`    @Max(${validation.max})`);
    }

    if (DECIMAL_TYPES.has(field.type)) {
        if (validation.decimalMin !== undefined) annotations.push(`    @DecimalMin("${escapeJavaString(validation.decimalMin)}")`);
        if (validation.decimalMax !== undefined) annotations.push(`    @DecimalMax("${escapeJavaString(validation.decimalMax)}")`);
        if (validation.digits) annotations.push(`    @Digits(integer = ${validation.digits.integer}, fraction = ${validation.digits.fraction})`);
    }

    if (INTEGER_TYPES.has(field.type) || DECIMAL_TYPES.has(field.type)) {
        if (validation.positive) annotations.push('    @Positive');
        if (validation.positiveOrZero) annotations.push('    @PositiveOrZero');
        if (validation.negative) annotations.push('    @Negative');
        if (validation.negativeOrZero) annotations.push('    @NegativeOrZero');
    }

    if (DATE_TYPES.has(field.type)) {
        if (validation.past) annotations.push('    @Past');
        if (validation.pastOrPresent) annotations.push('    @PastOrPresent');
        if (validation.future) annotations.push('    @Future');
        if (validation.futureOrPresent) annotations.push('    @FutureOrPresent');
    }

    if (field.type === 'Boolean') {
        if (validation.assertTrue) annotations.push('    @AssertTrue');
        if (validation.assertFalse) annotations.push('    @AssertFalse');
    }

    return annotations;
}

function renderDtoImports(fields, pkg) {
    const imports = new Set();

    for (const f of fields) {
        const validation = f.validation || {};

        if (f.required) {
            if (STRING_TYPES.has(f.type)) imports.add('jakarta.validation.constraints.NotBlank');
            else imports.add('jakarta.validation.constraints.NotNull');
        }

        const hasSize = STRING_TYPES.has(f.type) && (
            validation.minLength !== undefined ||
            validation.maxLength !== undefined ||
            (f.type === 'String' && f.length)
        );
        if (hasSize) imports.add('jakarta.validation.constraints.Size');

        if (validation.email) imports.add('jakarta.validation.constraints.Email');
        if (validation.pattern) imports.add('jakarta.validation.constraints.Pattern');

        if (validation.min !== undefined) imports.add('jakarta.validation.constraints.Min');
        if (validation.max !== undefined) imports.add('jakarta.validation.constraints.Max');

        if (validation.decimalMin !== undefined) imports.add('jakarta.validation.constraints.DecimalMin');
        if (validation.decimalMax !== undefined) imports.add('jakarta.validation.constraints.DecimalMax');
        if (validation.digits) imports.add('jakarta.validation.constraints.Digits');

        if (validation.positive) imports.add('jakarta.validation.constraints.Positive');
        if (validation.positiveOrZero) imports.add('jakarta.validation.constraints.PositiveOrZero');
        if (validation.negative) imports.add('jakarta.validation.constraints.Negative');
        if (validation.negativeOrZero) imports.add('jakarta.validation.constraints.NegativeOrZero');

        if (validation.past) imports.add('jakarta.validation.constraints.Past');
        if (validation.pastOrPresent) imports.add('jakarta.validation.constraints.PastOrPresent');
        if (validation.future) imports.add('jakarta.validation.constraints.Future');
        if (validation.futureOrPresent) imports.add('jakarta.validation.constraints.FutureOrPresent');

        if (validation.assertTrue) imports.add('jakarta.validation.constraints.AssertTrue');
        if (validation.assertFalse) imports.add('jakarta.validation.constraints.AssertFalse');
    }

    collectImports(fields, pkg).forEach((i) => imports.add(i));

    return imports.size ? [...imports].sort().map((i) => `import ${i};`).join('\n') + '\n' : '';
}

function renderResponseFields(fields) {
    return fields.map((f) => {
        if (f.type === 'ManyToOne') {
            return `    private String ${f.name}Id;`;
        }
        return `    private ${javaType(f)} ${f.name};`;
    }).join('\n');
}

function renderResponseImports(fields, pkg) {
    const typeImports = collectImports(fields, pkg);
    return formatImports(typeImports);
}

function renderEntityFields(fields) {
    const lines = [];
    for (const f of fields) {
        const annotations = [];

        if (f.filterable) {
            annotations.push('    @Filterable');
        }

        if (f.type === 'ManyToOne') {
            const fkColumn = _.snakeCase(f.name) + '_id';
            annotations.push(`    @ManyToOne(fetch = FetchType.LAZY)`);
            annotations.push(`    @JoinColumn(name = "${fkColumn}"${f.required ? ', nullable = false' : ''})`);
            lines.push(annotations.join('\n') + `\n    private ${f.refEntity}Entity ${f.name};`);
            continue;
        }

        const column = _.snakeCase(f.name);
        const colAttrs = [`name = "${column}"`];
        if (f.required) colAttrs.push('nullable = false');
        if (f.unique) colAttrs.push('unique = true');
        if (f.type === 'String' && f.length) colAttrs.push(`length = ${f.length}`);
        if (f.type === 'Text') colAttrs.push('columnDefinition = "TEXT"');
        if (f.type === 'MediumText') colAttrs.push('columnDefinition = "MEDIUMTEXT"');

        annotations.push(`    @Column(${colAttrs.join(', ')})`);

        if (f.type === 'Text' || f.type === 'MediumText') {
            annotations.push('    @Lob');
        }
        if (f.type === 'Enum') {
            annotations.push('    @Enumerated(EnumType.ORDINAL)');
        }

        const jType = f.type === 'Enum' ? f.enumName : TYPE_MAP[f.type].javaType;
        lines.push(annotations.join('\n') + `\n    private ${jType} ${f.name};`);
    }
    return lines.join('\n\n');
}

function renderEntityImports(fields, pkg) {
    const imports = new Set();
    for (const f of fields) {
        if (f.filterable) imports.add('gasi.gps.core.starter.infrastructure.filter.Filterable');
        if (f.type === 'ManyToOne') {
            imports.add('jakarta.persistence.ManyToOne');
            imports.add('jakarta.persistence.JoinColumn');
            imports.add('jakarta.persistence.FetchType');
        } else {
            imports.add('jakarta.persistence.Column');
            if (f.type === 'Text' || f.type === 'MediumText') imports.add('jakarta.persistence.Lob');
            if (f.type === 'Enum') {
                imports.add(`${pkg}.domain.model.${f.enumName}`);
                imports.add('jakarta.persistence.Enumerated');
                imports.add('jakarta.persistence.EnumType');
            }
            const mapping = TYPE_MAP[f.type];
            if (mapping) mapping.javaImports.forEach((i) => imports.add(i));
        }
    }
    return [...imports].sort().map((i) => `import ${i};`).join('\n') + '\n';
}

function renderDtoMapperMappings({ createFields, updateFields, summaryFields, detailFields }) {
    const createM2oFields = createFields.filter((f) => f.type === 'ManyToOne');
    const updateM2oFields = updateFields.filter((f) => f.type === 'ManyToOne');
    const summaryM2oFields = summaryFields.filter((f) => f.type === 'ManyToOne');
    const detailM2oFields = detailFields.filter((f) => f.type === 'ManyToOne');

    const toModelMappings = createM2oFields.map((f) =>
        `    @Mapping(target = "${f.name}", ignore = true)`,
    ).join('\n');

    const updateModelMappings = updateM2oFields.map((f) =>
        `    @Mapping(target = "${f.name}", ignore = true)`,
    ).join('\n');

    const summaryMappings = [
        '    @Mapping(target = "id", source = "id", qualifiedByName = "encodeId")',
        ...summaryM2oFields.map((f) =>
        `    @Mapping(source = "${f.name}.id", target = "${f.name}Id", qualifiedByName = "encodeId")`,
        ),
    ].join('\n');

    const detailMappings = [
        '    @Mapping(target = "id", source = "id", qualifiedByName = "encodeId")',
        ...detailM2oFields.map((f) =>
        `    @Mapping(source = "${f.name}.id", target = "${f.name}Id", qualifiedByName = "encodeId")`,
        ),
    ].join('\n');

    const hasMappings = toModelMappings || updateModelMappings || summaryMappings || detailMappings;
    const needsIdEncoderField = false;

    return {
        extraImports: hasMappings ? 'import org.mapstruct.Mapping;\n' : '',
        autowiredImport: needsIdEncoderField ? 'import org.springframework.beans.factory.annotation.Autowired;\n' : '',
        idEncoderField: needsIdEncoderField ? '    @Autowired\n    protected IdEncoder idEncoder;\n' : '',
        toModelMappings: toModelMappings ? toModelMappings + '\n' : '',
        updateModelMappings: updateModelMappings ? updateModelMappings + '\n' : '',
        summaryMappings: summaryMappings ? summaryMappings + '\n' : '',
        detailMappings: detailMappings ? detailMappings + '\n' : '',
    };
}

function renderMigrationColumns(fields) {
    if (!fields.length) {
        return '';
    }

    const columns = fields.map((f) => {
        const mapping = TYPE_MAP[f.type];
        let colName, colType;

        if (f.type === 'ManyToOne') {
            colName = _.snakeCase(f.name) + '_id';
            colType = mapping.sqlType();
        } else {
            colName = _.snakeCase(f.name);
            colType = mapping.sqlType(f);
        }

        const nullable = f.required ? ' NOT NULL' : '';
        const unique = f.unique ? ' UNIQUE' : '';
        return `    ${colName} ${colType}${nullable}${unique}`;
    });

    return ',\n' + columns.join(',\n');
}

function renderMigrationFkConstraints(fields, tableName, pluginPrefix) {
    const fks = fields.filter((f) => f.type === 'ManyToOne');
    if (!fks.length) return '';

    return '\n' + fks.map((f) => {
        const fkCol = _.snakeCase(f.name) + '_id';
        const refTable = buildTableName(pluginPrefix, f.refEntity);
        return `ALTER TABLE ${tableName}\n    ADD CONSTRAINT fk_${tableName}_${fkCol}\n    FOREIGN KEY (${fkCol}) REFERENCES ${refTable}(id);`;
    }).join('\n');
}

function renderServiceContext({ pkg, entityName, fields, mode = 'crud' }) {
    if (mode === 'read') {
        const imports = new Set([
            'org.springframework.stereotype.Service',
            'gasi.gps.core.starter.application.hook.ResourceMapperHookRegistry',
            'gasi.gps.core.starter.application.hook.ResourceServiceHookRegistry',
            'gasi.gps.core.starter.infrastructure.i18n.MessageUtil',
            'gasi.gps.core.starter.infrastructure.util.IdEncoder',
        ]);

        return {
            imports: [...imports].sort().map((i) => `import ${i};`).join('\n') + '\n',
            fields: '',
            constructorParams: renderServiceConstructorParams(entityName, [], false, true),
            constructorAssignments: '',
            superArgs: renderServiceSuperArgs(true),
        };
    }

    const imports = new Set([
        'org.springframework.stereotype.Service',
        'gasi.gps.core.starter.application.approval.ApprovalExtensionRegistry',
        'gasi.gps.core.starter.application.approval.ApprovalTargetHookRegistry',
        'gasi.gps.core.starter.application.hook.ResourceMapperHookRegistry',
        'gasi.gps.core.starter.application.hook.ResourceServiceHookRegistry',
        'gasi.gps.core.starter.infrastructure.i18n.MessageUtil',
        'gasi.gps.core.starter.infrastructure.util.IdEncoder',
    ]);

    return {
        imports: [...imports].sort().map((i) => `import ${i};`).join('\n') + '\n',
        fields: '',
        constructorParams: renderServiceConstructorParams(entityName, [], false, true, true),
        constructorAssignments: '',
        superArgs: renderServiceSuperArgs(true, true),
    };
}

function uniqueManyToOneFields(fields) {
    const byName = new Map();
    for (const field of fields) {
        byName.set(field.name, field);
    }
    return [...byName.values()];
}

function renderReferenceHookContext({ pkg, entityName, fields, mode = 'crud' }) {
    if (mode === 'read') {
        return emptyReferenceHookContext();
    }

    const createM2oFields = fieldsForDto(fields, 'create').filter((f) => f.type === 'ManyToOne');
    const updateM2oFields = fieldsForDto(fields, 'update').filter((f) => f.type === 'ManyToOne');
    const allM2oFields = uniqueManyToOneFields([...createM2oFields, ...updateM2oFields]);

    if (!allM2oFields.length) {
        return emptyReferenceHookContext();
    }

    const imports = new Set([
        'org.springframework.core.annotation.Order',
        'org.springframework.stereotype.Component',
        'gasi.gps.core.api.application.exception.BusinessException',
        'gasi.gps.core.api.application.hook.HookLayer',
        'gasi.gps.core.api.application.hook.ResourceHook',
        'gasi.gps.core.api.application.hook.ResourceServiceHook',
        'gasi.gps.core.starter.application.support.ReferenceResolver',
        `${pkg}.application.dto.${entityName}CreateRequest`,
        `${pkg}.application.dto.${entityName}DetailResponse`,
        `${pkg}.application.dto.${entityName}SummaryResponse`,
        `${pkg}.application.dto.${entityName}UpdateRequest`,
        `${pkg}.domain.model.${entityName}`,
    ]);

    for (const field of allM2oFields) {
        imports.add(`${pkg}.domain.model.${field.refEntity}`);
        imports.add(`${pkg}.domain.port.outbound.${field.refEntity}RepositoryPort`);
    }

    return {
        hasReferenceHook: true,
        imports: [...imports].sort().map((i) => `import ${i};`).join('\n') + '\n',
        fields: renderReferenceHookFields(allM2oFields),
        constructorParams: renderReferenceHookConstructorParams(allM2oFields),
        constructorAssignments: renderReferenceHookConstructorAssignments(allM2oFields),
        methods: renderReferenceHookMethods({ entityName, createM2oFields, updateM2oFields }),
    };
}

function emptyReferenceHookContext() {
    return {
        hasReferenceHook: false,
        imports: '',
        fields: '',
        constructorParams: '',
        constructorAssignments: '',
        methods: '',
    };
}

function renderReferenceHookFields(allM2oFields) {
    const lines = [
        '    private final ReferenceResolver referenceResolver;',
        ...uniqueRefEntities(allM2oFields).map((refEntity) =>
            `    private final ${refEntity}RepositoryPort ${_.lowerFirst(refEntity)}RepositoryPort;`),
    ];

    return lines.join('\n') + '\n\n';
}

function renderReferenceHookConstructorParams(allM2oFields) {
    return [
        'ReferenceResolver referenceResolver',
        ...uniqueRefEntities(allM2oFields).map((refEntity) =>
            `${refEntity}RepositoryPort ${_.lowerFirst(refEntity)}RepositoryPort`),
    ].join(',\n            ');
}

function renderServiceConstructorParams(
    entityName,
    allM2oFields,
    hasRefs,
    includeHookRegistry = false,
    includeApprovalRegistry = false,
) {
    const params = [
        `${entityName}RepositoryPort repositoryPort`,
        `${entityName}DtoMapper dtoMapper`,
        'MessageUtil messageUtil',
        'IdEncoder idEncoder',
    ];

    if (includeHookRegistry) {
        params.push('ResourceServiceHookRegistry hookRegistry');
        params.push('ResourceMapperHookRegistry mapperHookRegistry');
    }

    if (includeApprovalRegistry) {
        params.push('ApprovalExtensionRegistry approvalExtensionRegistry');
        params.push('ApprovalTargetHookRegistry approvalTargetHookRegistry');
    }

    if (hasRefs) {
        params.push('ReferenceResolver referenceResolver');
        uniqueRefEntities(allM2oFields).forEach((refEntity) => {
            params.push(`${refEntity}RepositoryPort ${_.lowerFirst(refEntity)}RepositoryPort`);
        });
    }

    return params.join(',\n            ');
}

function renderServiceSuperArgs(includeHookRegistry = false, includeApprovalRegistry = false) {
    const args = ['repositoryPort', 'dtoMapper', 'messageUtil', 'idEncoder'];
    if (includeHookRegistry) {
        args.push('hookRegistry');
        args.push('mapperHookRegistry');
    }
    if (includeApprovalRegistry) {
        args.push('approvalExtensionRegistry');
        args.push('approvalTargetHookRegistry');
    }
    return args.join(', ');
}

function renderReferenceHookConstructorAssignments(allM2oFields) {
    const lines = [
        '        this.referenceResolver = referenceResolver;',
        ...uniqueRefEntities(allM2oFields).map((refEntity) =>
            `        this.${_.lowerFirst(refEntity)}RepositoryPort = ${_.lowerFirst(refEntity)}RepositoryPort;`),
    ];

    return lines.join('\n') + '\n';
}

function renderReferenceHookMethods({ entityName, createM2oFields, updateM2oFields }) {
    const entityVar = _.lowerFirst(entityName);
    return `
    @Override
    public void beforeCreate(String resourceType, ${entityName} ${entityVar}, ${entityName}CreateRequest request) {
        BusinessException.Collector collector = new BusinessException.Collector();
${renderResolveAssignments(createM2oFields)}
        collector.validate();
${renderApplyRefs(entityVar, createM2oFields)}
    }

    @Override
    public void beforeUpdate(String resourceType, ${entityName} ${entityVar}, ${entityName}UpdateRequest request) {
        BusinessException.Collector collector = new BusinessException.Collector();
${renderResolveAssignments(updateM2oFields)}
        collector.validate();
${renderApplyRefs(entityVar, updateM2oFields)}
    }
`;
}

function renderResolveAssignments(fields) {
    if (!fields.length) {
        return '';
    }
    return fields.map((field) => `        ${field.refEntity} ${field.name} = referenceResolver.resolve(
                ${_.lowerFirst(field.refEntity)}RepositoryPort,
                request.get${_.upperFirst(field.name)}Id(),
                "${field.name}Id",
                collector);`).join('\n');
}

function renderApplyRefs(entityVar, fields) {
    if (!fields.length) {
        return '';
    }
    return fields.map((field) => `        ${entityVar}.set${_.upperFirst(field.name)}(${field.name});`).join('\n');
}

function uniqueRefEntities(fields) {
    return [...new Set(fields.map((field) => field.refEntity))];
}

// ─── Main Context Builder ──────────────────────────────────────────────

/**
 * Build the full token context for template rendering.
 *
 * @param {object} params - { pkg, entityName, tableName, fields, pluginName, domain }
 * @returns {object} context with all tokens needed by templates/api/resource/
 */
function buildResourceContext({ pkg, entityName, tableName, fields, pluginName, pluginPrefix, domain, parent, apiStyle, embedInParentDto, ui, mode = 'crud' }) {
    const entityVar = _.lowerFirst(entityName);
    const apiPath = pluralize(_.snakeCase(entityName).replace(/_/g, '-'));
    const parentApiPath = parent ? pluralize(_.snakeCase(parent).replace(/_/g, '-')) : '';
    const parentVar = parent ? _.lowerFirst(parent) : '';
    const parentIdParam = parentVar ? `${parentVar}Id` : '';
    const isNestedApi = Boolean(parent && apiStyle === 'nested');
    // PKG_PATH is base package only (e.g. "gasi/gps"), DOMAIN is appended via [[DOMAIN]] in template path
    const basePkg = pkg.substring(0, pkg.lastIndexOf('.'));
    const pkgPath = basePkg.replace(/\./g, '/');

    const createFields = fieldsForDto(fields, 'create');
    const updateFields = fieldsForDto(fields, 'update');
    const summaryFields = fieldsForDto(fields, 'summary');
    const detailFields = fieldsForDto(fields, 'detail');

    const mapperCtx = renderDtoMapperMappings({
        createFields,
        updateFields,
        summaryFields,
        detailFields,
    });
    const serviceCtx = renderServiceContext({ pkg, entityName, fields, mode });
    const referenceHookCtx = renderReferenceHookContext({ pkg, entityName, fields, mode });
    const isReadMode = mode === 'read';
    const controllerCtx = renderControllerContext({
        pkg,
        entityName,
        entityVar,
        apiPath,
        parent,
        parentApiPath,
        parentIdParam,
        isNestedApi,
        isReadMode,
        summaryFields,
        lookupFields: fieldsForLookup(summaryFields, ui?.lookup),
    });

    return {
        // Path tokens ([[TOKEN]] in directory/file names)
        PKG_PATH: pkgPath,
        DOMAIN: domain,
        ENTITY_NAME: entityName,
        TABLE_NAME: tableName,
        PLUGIN_NAME: pluginName,
        PLUGIN_PREFIX: pluginPrefix,
        MIGRATION_TIMESTAMP: nowTimestamp(),

        // Content tokens ({{TOKEN}} inside file contents)
        FULL_PACKAGE: pkg,
        ENTITY_VAR: entityVar,
        API_PATH: apiPath,
        CONTROLLER_API_PATH: controllerCtx.apiPath,
        PARENT_ENTITY_NAME: parent || '',
        PARENT_API_PATH: parentApiPath,
        API_STYLE: apiStyle || 'root',
        RESOURCE_MODE: mode,
        EMBED_IN_PARENT_DTO: embedInParentDto ? 'true' : 'false',
        CONTROLLER_DEFAULT_SUMMARY_FIELDS: renderControllerDefaultSummaryFields(summaryFields),
        CONTROLLER_DEFAULT_LOOKUP_FIELDS: renderControllerDefaultLookupFields(fieldsForLookup(summaryFields, ui?.lookup)),

        CONTROLLER_BASE_IMPORT: isNestedApi ? '' : isReadMode
            ? 'import gasi.gps.core.starter.presentation.controller.BaseReadController;'
            : 'import gasi.gps.core.starter.presentation.controller.BaseController;',
        CONTROLLER_REQUEST_IMPORTS: isReadMode ? '' : `import ${pkg}.application.dto.${entityName}CreateRequest;
import ${pkg}.application.dto.${entityName}UpdateRequest;
import ${pkg}.domain.model.${entityName};`,
        CONTROLLER_EXTENDS: isReadMode
            ? `BaseReadController<${entityName}SummaryResponse, ${entityName}DetailResponse>`
            : `BaseController<${entityName}, ${entityName}CreateRequest, ${entityName}UpdateRequest, ${entityName}SummaryResponse, ${entityName}DetailResponse>`,
        CONTROLLER_NESTED_IMPORTS: controllerCtx.nestedImports,
        CONTROLLER_CLASS_EXTENDS: controllerCtx.classExtends,
        CONTROLLER_BODY: controllerCtx.body,

        INBOUND_BASE_IMPORT: isReadMode
            ? 'import gasi.gps.core.api.domain.port.inbound.BaseReadService;'
            : 'import gasi.gps.core.api.domain.port.inbound.BaseService;',
        INBOUND_REQUEST_IMPORTS: isReadMode ? '' : `import ${pkg}.application.dto.${entityName}CreateRequest;
import ${pkg}.application.dto.${entityName}UpdateRequest;
import ${pkg}.domain.model.${entityName};`,
        INBOUND_EXTENDS: isReadMode
            ? `BaseReadService<${entityName}SummaryResponse, ${entityName}DetailResponse>`
            : `BaseService<${entityName}, ${entityName}CreateRequest, ${entityName}UpdateRequest, ${entityName}SummaryResponse, ${entityName}DetailResponse>`,

        SERVICE_BASE_IMPORT: isReadMode
            ? 'import gasi.gps.core.starter.application.service.BaseReadServiceImpl;'
            : 'import gasi.gps.core.starter.application.service.BaseServiceImpl;',
        SERVICE_AUDIT_IMPORT: isReadMode
            ? ''
            : 'import gasi.gps.core.api.audit.AuditableEntity;',
        SERVICE_AUDIT_ANNOTATION: isReadMode
            ? ''
            : `@AuditableEntity(module = "${escapeJavaString(domain)}", resourceType = "${escapeJavaString(entityName)}")`,
        SERVICE_REQUEST_IMPORTS: isReadMode ? '' : `import ${pkg}.application.dto.${entityName}CreateRequest;
import ${pkg}.application.dto.${entityName}UpdateRequest;`,
        SERVICE_EXTENDS: isReadMode
            ? `BaseReadServiceImpl<${entityName}, ${entityName}SummaryResponse, ${entityName}DetailResponse>`
            : `BaseServiceImpl<${entityName}, ${entityName}CreateRequest, ${entityName}UpdateRequest, ${entityName}SummaryResponse, ${entityName}DetailResponse>`,

        DTO_MAPPER_BASE_IMPORT: isReadMode
            ? 'import gasi.gps.core.starter.application.mapper.BaseReadDtoMapper;'
            : 'import gasi.gps.core.starter.application.mapper.BaseDtoMapper;',
        DTO_MAPPER_REQUEST_IMPORTS: isReadMode ? '' : `import ${pkg}.application.dto.${entityName}CreateRequest;
import ${pkg}.application.dto.${entityName}UpdateRequest;`,
        DTO_MAPPER_MAPPING_TARGET_IMPORT: isReadMode ? '' : 'import org.mapstruct.MappingTarget;',
        DTO_MAPPER_BASE_INTERFACE: isReadMode
            ? `BaseReadDtoMapper<${entityName}, ${entityName}SummaryResponse, ${entityName}DetailResponse>`
            : `BaseDtoMapper<${entityName}, ${entityName}CreateRequest, ${entityName}UpdateRequest, ${entityName}SummaryResponse, ${entityName}DetailResponse>`,
        DTO_MAPPER_WRITE_METHODS: isReadMode ? '' : `${mapperCtx.toModelMappings}
    @Override
    public abstract ${entityName} toCreateDomain(${entityName}CreateRequest request);

${mapperCtx.updateModelMappings}
    @Override
    public abstract void updateDomain(${entityName}UpdateRequest request, @MappingTarget ${entityName} model);
`,

        // Domain model
        DOMAIN_MODEL_IMPORTS: formatImports(collectImports(fields)),
        DOMAIN_MODEL_FIELDS: renderDomainModelFields(fields),

        // DTOs — request
        CREATE_REQUEST_IMPORTS: renderDtoImports(createFields, pkg),
        CREATE_REQUEST_FIELDS: renderDtoFields(createFields),
        UPDATE_REQUEST_IMPORTS: renderDtoImports(updateFields, pkg),
        UPDATE_REQUEST_FIELDS: renderDtoFields(updateFields),

        // DTOs — response
        SUMMARY_RESPONSE_IMPORTS: renderResponseImports(summaryFields, pkg),
        SUMMARY_RESPONSE_FIELDS: renderResponseFields(summaryFields),
        DETAIL_RESPONSE_IMPORTS: renderResponseImports(detailFields, pkg),
        DETAIL_RESPONSE_FIELDS: renderResponseFields(detailFields),

        // DTO Mapper
        DTO_MAPPER_EXTRA_IMPORTS: mapperCtx.extraImports,
        DTO_MAPPER_AUTOWIRED_IMPORT: mapperCtx.autowiredImport,
        DTO_MAPPER_ID_ENCODER_FIELD: mapperCtx.idEncoderField,
        DTO_MAPPER_TO_MODEL_MAPPINGS: mapperCtx.toModelMappings,
        DTO_MAPPER_UPDATE_MODEL_MAPPINGS: mapperCtx.updateModelMappings,
        DTO_MAPPER_SUMMARY_MAPPINGS: mapperCtx.summaryMappings,
        DTO_MAPPER_DETAIL_MAPPINGS: mapperCtx.detailMappings,
        DTO_MAPPER_CHILD_IMPORTS: '',
        DTO_MAPPER_CHILD_METHODS: '',

        // Service
        SERVICE_IMPORTS: serviceCtx.imports,
        SERVICE_FIELDS: serviceCtx.fields,
        SERVICE_CONSTRUCTOR_PARAMS: serviceCtx.constructorParams,
        SERVICE_CONSTRUCTOR_ASSIGNMENTS: serviceCtx.constructorAssignments,
        SERVICE_SUPER_ARGS: serviceCtx.superArgs,

        // Reference hook
        HAS_REFERENCE_HOOK: referenceHookCtx.hasReferenceHook ? 'true' : '',
        REFERENCE_HOOK_IMPORTS: referenceHookCtx.imports,
        REFERENCE_HOOK_FIELDS: referenceHookCtx.fields,
        REFERENCE_HOOK_CONSTRUCTOR_PARAMS: referenceHookCtx.constructorParams,
        REFERENCE_HOOK_CONSTRUCTOR_ASSIGNMENTS: referenceHookCtx.constructorAssignments,
        REFERENCE_HOOK_METHODS: referenceHookCtx.methods,

        // JPA Entity
        ENTITY_IMPORTS: renderEntityImports(fields, pkg),
        ENTITY_FIELDS: renderEntityFields(fields),

        // Migration
        MIGRATION_COLUMNS: renderMigrationColumns(fields),
        MIGRATION_FK_CONSTRAINTS: renderMigrationFkConstraints(fields, tableName, pluginPrefix),
    };
}

function responseFieldName(field) {
    return field.type === 'ManyToOne' ? `${field.name}Id` : field.name;
}

function renderControllerDefaultSummaryFields(summaryFields) {
    const defaultFields = summaryFields.filter((f) => f.defaultColumn !== false);
    const fields = ['id', ...defaultFields.map((field) => responseFieldName(field))];
    const uniqueFields = [...new Set(fields)];
    return uniqueFields.map((field) => `"${field}"`).join(', ');
}

function fieldsForLookup(summaryFields, lookupConfig) {
    if (lookupConfig) {
        const configuredNames = [
            ...(lookupConfig.labelFields ?? []),
            ...(lookupConfig.descriptionFields ?? []),
            ...(lookupConfig.columns ?? []).map((column) => column.field),
        ];
        const configured = configuredNames
            .map((name) => summaryFields.find((field) => responseFieldName(field) === name))
            .filter(Boolean);

        if (configured.length) {
            return configured;
        }
    }

    const available = new Map(summaryFields.map((field) => [responseFieldName(field), field]));
    const preferredNames = ['code', 'name', 'fullName', 'employeeNo'];
    const preferred = preferredNames
        .map((name) => available.get(name))
        .filter(Boolean);

    if (preferred.length) {
        return preferred;
    }

    return summaryFields
        .filter((field) => responseFieldName(field) !== 'id')
        .slice(0, 2);
}

function renderControllerDefaultLookupFields(lookupFields) {
    const fields = ['id', ...lookupFields.map((field) => responseFieldName(field))];
    const uniqueFields = [...new Set(fields)];
    return uniqueFields.map((field) => `"${field}"`).join(', ');
}

function renderControllerContext({
    entityName,
    entityVar,
    apiPath,
    parent,
    parentApiPath,
    parentIdParam,
    isNestedApi,
    isReadMode,
    summaryFields,
    lookupFields,
}) {
    if (!isNestedApi) {
        return {
            apiPath,
            nestedImports: '',
            classExtends: isReadMode
                ? `\n        extends BaseReadController<${entityName}SummaryResponse, ${entityName}DetailResponse>`
                : `\n        extends BaseController<${entityName}, ${entityName}CreateRequest, ${entityName}UpdateRequest, ${entityName}SummaryResponse, ${entityName}DetailResponse>`,
            body: `    public ${entityName}Controller(${entityName}Service ${entityVar}Service, IdEncoder idEncoder,
            ResourceControllerHookRegistry hookRegistry) {
        super(${entityVar}Service, idEncoder, hookRegistry);
    }

    @Override
    public String getResourceName() {
        return "${entityName}";
    }

    @Override
    protected List<String> getDefaultSummaryFields() {
        return List.of(${renderControllerDefaultSummaryFields(summaryFields)});
    }

    @Override
    protected List<String> getDefaultLookupFields() {
        return List.of(${renderControllerDefaultLookupFields(lookupFields)});
    }`,
        };
    }

    const parentField = _.lowerFirst(parent);
    const parentGetter = `get${_.upperFirst(parentField)}Id`;
    const parentSetter = `set${_.upperFirst(parentField)}Id`;
    const nestedPath = `${parentApiPath}/{${parentIdParam}}/${apiPath}`;
    const writeMethods = isReadMode ? '' : `

    @PostMapping
    @ResponseStatus(HttpStatus.OK)
    @PreAuthorize("hasPermission(this, 'CREATE')")
    public ApiResponse<${entityName}DetailResponse> create(
            @PathVariable String ${parentIdParam},
            @Valid @RequestBody ${entityName}CreateRequest request) {
        request.${parentSetter}(${parentIdParam});
        return ApiResponse.ok(${entityVar}Service.create(request));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasPermission(this, 'UPDATE')")
    public ApiResponse<${entityName}DetailResponse> update(
            @PathVariable String ${parentIdParam},
            @PathVariable String id,
            @Valid @RequestBody ${entityName}UpdateRequest request) {
        ensureBelongsToParent(${parentIdParam}, id);
        request.${parentSetter}(${parentIdParam});
        return ApiResponse.ok(${entityVar}Service.update(idEncoder.decode(id), request));
    }

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @PreAuthorize("hasPermission(this, 'DELETE')")
    public ApiResponse<Void> delete(@PathVariable String ${parentIdParam}, @PathVariable String id) {
        ensureBelongsToParent(${parentIdParam}, id);
        ${entityVar}Service.delete(idEncoder.decode(id));
        return ApiResponse.noContent();
    }`;

    return {
        apiPath: nestedPath,
        nestedImports: `import java.util.Collections;
import org.springframework.http.HttpStatus;
import org.springframework.security.access.prepost.PreAuthorize;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.ResponseStatus;
import gasi.gps.core.api.application.exception.EntityNotFoundException;
import gasi.gps.core.api.domain.model.AndFilter;
import gasi.gps.core.api.domain.model.GenericFilter;
import gasi.gps.core.api.domain.model.PageResult;
import gasi.gps.core.api.domain.model.SimpleFilter;
import gasi.gps.core.api.presentation.dto.ApiResponse;
import gasi.gps.core.api.presentation.dto.SearchRequest;
import gasi.gps.core.starter.presentation.support.ResponseProjection;
import jakarta.validation.Valid;`,
        classExtends: '',
        body: `    private final ${entityName}Service ${entityVar}Service;
    private final IdEncoder idEncoder;

    public ${entityName}Controller(${entityName}Service ${entityVar}Service, IdEncoder idEncoder) {
        this.${entityVar}Service = ${entityVar}Service;
        this.idEncoder = idEncoder;
    }

    public String getResourceName() {
        return "${entityName}";
    }

    protected List<String> getDefaultSummaryFields() {
        return List.of(${renderControllerDefaultSummaryFields(summaryFields)});
    }

    private List<String> resolveProjectionFields(SearchRequest request) {
        return request.getFields() == null || request.getFields().isEmpty()
                ? getDefaultSummaryFields()
                : request.getFields();
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasPermission(this, 'READ')")
    public ApiResponse<${entityName}DetailResponse> findById(@PathVariable String ${parentIdParam}, @PathVariable String id) {
        ${entityName}DetailResponse response = ${entityVar}Service.findById(idEncoder.decode(id));
        ensureBelongsToParent(${parentIdParam}, id, response);
        return ApiResponse.ok(response);
    }

    @PostMapping("/search")
    @PreAuthorize("hasPermission(this, 'READ')")
    public ApiResponse<${entityName}DetailResponse> findBy(
            @PathVariable String ${parentIdParam},
            @RequestBody SearchRequest request) {
        return ApiResponse.ok(${entityVar}Service.findBy(withParentFilter(${parentIdParam}, request).getFilter()));
    }

    @PostMapping("/search/list")
    @PreAuthorize("hasPermission(this, 'READ')")
    public ApiResponse<List<?>> findAll(
            @PathVariable String ${parentIdParam},
            @RequestBody SearchRequest request) {
        SearchRequest scopedRequest = withParentFilter(${parentIdParam}, request);
        List<${entityName}SummaryResponse> result = ${entityVar}Service.findAll(
                scopedRequest.getFilter(),
                scopedRequest.getSorts() != null ? scopedRequest.getSorts() : Collections.emptyList());
        return ApiResponse.ok(ResponseProjection.projectList(result, resolveProjectionFields(scopedRequest)));
    }

    @PostMapping("/search/page")
    @PreAuthorize("hasPermission(this, 'READ')")
    public ApiResponse<PageResult<?>> findAllPaged(
            @PathVariable String ${parentIdParam},
            @RequestBody SearchRequest request) {
        SearchRequest scopedRequest = withParentFilter(${parentIdParam}, request);
        PageResult<${entityName}SummaryResponse> result = ${entityVar}Service.findAll(
                scopedRequest.getPage() != null ? scopedRequest.getPage() : 0,
                scopedRequest.getSize() != null ? scopedRequest.getSize() : 10,
                scopedRequest.getFilter(),
                scopedRequest.getSorts() != null ? scopedRequest.getSorts() : Collections.emptyList());
        return ApiResponse.ok(ResponseProjection.projectPage(result, resolveProjectionFields(scopedRequest)));
    }${writeMethods}

    private SearchRequest withParentFilter(String ${parentIdParam}, SearchRequest request) {
        SearchRequest scopedRequest = request != null ? request : new SearchRequest();
        GenericFilter parentFilter = SimpleFilter.builder()
                .field("${parentField}.id")
                .operator(SimpleFilter.FilterOperator.EQUALS)
                .value(idEncoder.decode(${parentIdParam}))
                .build();

        if (scopedRequest.getFilter() == null) {
            scopedRequest.setFilter(parentFilter);
        } else {
            scopedRequest.setFilter(AndFilter.builder()
                    .filters(List.of(parentFilter, scopedRequest.getFilter()))
                    .build());
        }

        return scopedRequest;
    }

    private void ensureBelongsToParent(String ${parentIdParam}, String id) {
        ensureBelongsToParent(${parentIdParam}, id, ${entityVar}Service.findById(idEncoder.decode(id)));
    }

    private void ensureBelongsToParent(String ${parentIdParam}, String id, ${entityName}DetailResponse response) {
        if (response == null || response.${parentGetter}() == null || !response.${parentGetter}().equals(${parentIdParam})) {
            throw new EntityNotFoundException("${entityName} not found: " + id);
        }
    }`,
    };
}

function nowTimestamp() {
    const d = new Date();
    d.setMilliseconds(0);
    const current = d.getTime();
    const next = current <= nowTimestamp.last ? nowTimestamp.last + 1000 : current;
    nowTimestamp.last = next;
    return formatTimestamp(new Date(next));
}

function formatTimestamp(d) {
    const pad = (n) => String(n).padStart(2, '0');
    return (
        d.getFullYear().toString() +
        pad(d.getMonth() + 1) +
        pad(d.getDate()) +
        pad(d.getHours()) +
        pad(d.getMinutes()) +
        pad(d.getSeconds())
    );
}

nowTimestamp.last = 0;

module.exports = { buildResourceContext, TYPE_MAP };
