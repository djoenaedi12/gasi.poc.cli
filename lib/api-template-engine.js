...
/**
 * Walk a scaffold template directory and copy every file to the target directory,
...
async function renderTemplateTree(templateRoot, targetRoot, ctx, opts = {}) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory) {
            ...
            const replaced = replaceContentTokens(content, ctx);
            const transformed = typeof opts.transformContent === 'function'
                ? opts.transformContent(replaced, renderedRelPath)
                : replaced;
            const rendered = normalizeRenderedContent(transformed, renderedRelPath);
