...
function normalizePluginModuleName(name) {
    ...
    if (!raw) throw new Error('Plugin module is required.');

    const normalizedPath = raw.replace(/\\/g, '/').replace(/^\.\/+/, '').replace(/\/+$/, '');
    return normalizedPath.startsWith('plugins/')
        ? normalizedPath
        : `plugins/${normalizePluginName(normalizedPath)}`;
}

function resolvePluginModuleName(pluginOption, pluginModules) {
    const raw = String(pluginOption || '').trim();
    if (!raw) throw new Error('--plugin is required.');

    const normalizedPath = raw.replace(/\\/g, '/').replace(/^\.\/+/, '').replace(/\/+$/, '');
    const candidates = [...new Set([
        normalizedPath,
        normalizePluginModuleName(normalizedPath),
    ])];

    const match = candidates.find((candidate) => pluginModules.includes(candidate));
    if (match) {
        return match;
    }

    throw new Error(`Plugin "${pluginOption}" is not registered in pom.xml. Available plugins: ${pluginModules.join(', ')}`);
}
...
/**
 * Cari file UMD hasil build di dist/ folder plugin.
...
async function findWebBundle(pluginDir, pluginName) {
    ...
    throw new Error(`Bundle not found in ${path.join(pluginDir, 'dist')}. Expected ${fileNames.join(' or ')}. Build the plugin first with: gasi plugin build ${name} --target web`);
}
