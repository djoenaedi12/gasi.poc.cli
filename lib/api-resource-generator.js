...
async function generateApiResource({
    ...
    const shouldInclude = (relPath) => shouldIncludeTemplate(relPath, effectiveExposeApi, mode);
...
async function generateApiEnums({ pluginDir, enums }) {
    if (!enums.length) {
        return [];
    }
    ...
    for (const enumSpec of enums) {
        ...
        await fs.writeFile(targetPath, renderEnumFile(pkg, enumSpec), 'utf8');
...
function shouldIncludeTemplate(relPath, exposeApi, mode = 'crud') {
    if (exposeApi) {
