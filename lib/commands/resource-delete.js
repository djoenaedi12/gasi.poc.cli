...
async function resolvePluginPrefix(pluginDir) {
    const propsFile = path.join(pluginDir, 'src', 'main', 'resources', 'plugin.properties');
    if (await fs.pathExists(propsFile)) {
        const content = await fs.readFile(propsFile, 'utf8');
        const match = content.match(/^plugin\.prefix\s*=\s*(.*)$/m);
        return match ? _.snakeCase(match[1].trim()).toLowerCase() : '';
    }
...
async function resourceDelete(entityName, opts) {
    ...
    let pluginModule, pluginDir, pkg, srcRoot, projectRoot, pluginPrefix;
    ...
    if (detected) {
        ...
        pluginPrefix = await resolvePluginPrefix(pluginDir);
    ...
    if (opts.includeMigration) {
        const tableName = buildTableName(pluginPrefix, entityName);
    ...
    const spinner = ora('Deleting API resource files...').start();
    try {
        for (const f of existing) {
            await fs.remove(f);
        }
        ...
        spinner.fail('Failed to delete API resource files.');
        throw err;
    }
