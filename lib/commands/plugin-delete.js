...
async function pluginDelete(pluginName, opts) {
    ...
    if (doWeb) {
        try {
            ...
            const manifestUrls  = opts.keepDeployed ? [] : getWebPluginManifestUrls(pluginName);
            const manifestMatches = opts.keepDeployed ? [] : await findWebPluginManifestMatches(webPluginsDir, manifestUrls);

            if (!pluginDirExists && !bundle && !manifestMatches.length) {
                throw new Error(`Web plugin directory does not exist: ${pluginDir}`);
            }

            plan.web = { pluginId, pluginDir, webPluginsDir, bundle, manifestUrls, manifestMatches };
        } catch (err) {
    ...
    if (plan.web) {
        ...
        try {
            ...
            await removeFromWebPluginManifest(plan.web.webPluginsDir, plan.web.manifestUrls);
...
function getWebPluginManifestUrls(pluginName) {
    const pluginId = normalizePluginName(pluginName);
    const name = pluginId.replace('-plugin', '');
    return [
        `/plugins/plugin-${name}.umd.cjs`,
        `/plugins/plugin-${name}.umd.js`,
    ];
}

async function removeFromWebPluginManifest(webPluginsDir, pluginUrls) {
    ...
    const removeNames = new Set(pluginUrls.map((url) => path.basename(url)));
    const next = current.filter((entry) => {
        const url = typeof entry === 'string' ? entry : entry?.url;
        return !removeNames.has(path.basename(url || ''));
    });

    await fs.writeJson(manifestPath, next, { spaces: 2 });
}

async function findWebPluginManifestMatches(webPluginsDir, pluginUrls) {
    const manifestPath = path.join(webPluginsDir, 'manifest.json');
    if (!(await fs.pathExists(manifestPath))) return [];
    ...
    const matchNames = new Set(pluginUrls.map((url) => path.basename(url)));
    return current
        .map((entry) => typeof entry === 'string' ? entry : entry?.url)
        .filter((url) => typeof url === 'string' && matchNames.has(path.basename(url)));
}
...
function printPlan(plan, cwd) {
    ...
    if (plan.web) {
        console.log(chalk.bold('\n  Web:'));
        if (plan.web.pluginDir) {
            console.log(`    Remove dir      : ${path.relative(cwd, plan.web.pluginDir)}`);
        }
        if (plan.web.bundle) {
        ...
        if (plan.web.manifestMatches.length) {
            plan.web.manifestMatches.forEach((url) => console.log(`    Remove manifest : ${url}`));
        } else {
            console.log(`    Remove manifest : (none registered)`);
        }
    }
