...
async function pluginCreate(opts) {
    ...
    if (doWeb) {
        ...
        console.log(chalk.gray(`  ${step++}.`) + ` Register in platform-app/src/main.tsx: ${chalk.cyan(`'/plugins/plugin-${answers.name}.umd.js'`)}`);
...
async function promptPluginDependencies(parentPomPath, ownPluginName, preSelectedFlags) {
    ...
    if (known.length > 0) {
        const { selected } = await inquirer.prompt([{
            ...
            choices: known.map((id) => ({ name: id, value: id, checked: preIds.has(id) })),
        }]);
    ...
    for (const dep of preSelectedFlags) {
        ...
        if (!result.includes(dep) && !result.includes(baseId)) result.push(dep);
    }
