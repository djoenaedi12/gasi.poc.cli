...
async function generateWebTarget({ entityName, opts, cwd, filePaths, resourceSpecs }) {
    ...
    try {
        ...
        for (const file of generatedFiles) {
            console.log(chalk.gray('    ' + path.relative(webDir, file)));
        }

        console.log(chalk.green.bold('\n✓ Web resource created successfully!\n'));
        console.log(chalk.bold('Next steps:'));
        console.log(chalk.gray('  1.') + ' Review generated forms, columns, and routes');
        console.log(chalk.gray('  2.') + ' Import generated routes into your app router');
        console.log(chalk.gray('  3.') + ' Wire service calls to your query/mutation layer\n');
    } catch (err) {
...
async function resourceCreate(entityName, opts) {
    ...
    if (target === 'all') {
        await generateWebTarget({ entityName, opts, cwd, filePaths, resourceSpecs });
    }
}
