...
function buildWebResourceContext(resource) {
    ...
    return {
        ...
        DETAIL_CORE_API_IMPORTS: isReadMode ? '' : 'import { Actions } from "@gasi/core-api";',
        DETAIL_STORE_IMPORT: isReadMode ? '' : 'import { useAppStore } from "@gasi/core-starter";',
        ...
        DETAIL_PERMISSION_SETUP: isReadMode ? '' : `    const hasPermission = useAppStore((state) => state.hasPermission);
    const canUpdate = hasPermission(\`${entityVar}:\${Actions.UPDATE}\`);`,
...
function advancedFilterFields(ctx) {
    return ctx.filterFields.filter((field) => !['String', 'Text', 'MediumText'].includes(field.type));
}
...
function renderRouteDefinitions(ctx) {
    const lines = [
        `    { path: "${ctx.routePath}",         component: ${ctx.entityName}ListPage, resource: "${ctx.entityVar}", action: Actions.READ },`,
    ];

    if (!isReadMode(ctx)) {
        lines.push(`    { path: "${ctx.routePath}/create",  component: ${ctx.entityName}CreatePage, resource: "${ctx.entityVar}", action: Actions.CREATE },`);
    }

    lines.push(`    { path: "${ctx.routePath}/:id",     component: ${ctx.entityName}DetailPage, resource: "${ctx.entityVar}", action: Actions.READ },`);

    if (!isReadMode(ctx)) {
        lines.push(`    { path: "${ctx.routePath}/:id/edit", component: ${ctx.entityName}EditPage, resource: "${ctx.entityVar}", action: Actions.UPDATE },`);
    }
...
function renderListCoreApiImports(ctx) {
    return isReadMode(ctx) ? '' : 'import { Actions } from "@gasi/core-api";';
}

function renderListStoreImport(ctx) {
    return isReadMode(ctx) ? '' : 'import { useAppStore } from "@gasi/core-starter";';
}
...
function renderListPermissions(ctx) {
    ...
    return `    const hasPermission = useAppStore((state) => state.hasPermission);
    const delete${ctx.entityName} = useDelete${ctx.entityName}(${ctx.isNestedApi ? `${ctx.parentParam} ?? ""` : ''});
    const canCreate = hasPermission(\`${ctx.entityVar}:\${Actions.CREATE}\`);
    const canUpdate = hasPermission(\`${ctx.entityVar}:\${Actions.UPDATE}\`);
    const canDelete = hasPermission(\`${ctx.entityVar}:\${Actions.DELETE}\`);
    const canDownload = hasPermission(\`${ctx.entityVar}:\${Actions.DOWNLOAD}\`);
    const canUpload = hasPermission(\`${ctx.entityVar}:\${Actions.UPLOAD}\`);`;
}
...
function renderListDeleteHandlers(ctx) {
    ...
    const handleDelete = useCallback((id: string) => {
        delete${ctx.entityName}.mutate(id);
    }, [delete${ctx.entityName}]);

    const handleBulkDelete = useCallback((ids: string[]) => {
        ids.forEach((id) => delete${ctx.entityName}.mutate(id));
    }, [delete${ctx.entityName}]);
`;
}
...
function renderListBulkUploadHandler(ctx) {
    ...
    const handleBulkUpload = useCallback(() => {
        console.log("Open ${ctx.entityPluralPascal} bulk upload flow");
    }, []);
`;
}
...
function renderListColumnsDeps(ctx) {
    if (isReadMode(ctx)) {
        return ctx.isNestedApi ? '[listPath]' : '[]';
    }

    return '[canDelete, canUpdate, handleDelete]';
}
...
function renderColumnActionItem(ctx) {
    if (isReadMode(ctx)) {
        ...
        getDataTableRowActionsColumn({
            basePath,
            entityName: "${_.startCase(ctx.entityVar)}",
...
function renderAdvancedFilterControls(ctx) {
    return advancedFilterFields(ctx).map(renderAdvancedFilterControl).join('\n\n');
}

function renderAdvancedFilterControl(field) {
    const stateName = advancedFilterStateName(field);
...
function renderMoreFilterProps(ctx) {
    ...
    return `advancedFilter={advancedFilter}
...
function renderBreadcrumbLabel(ctx) {
    ...
    if (!identifierFields.length) {
        return `params.id ?? "${ctx.entityName}"`;
    }
    ...
    return `[${values}].filter(Boolean).join(" - ") || params.id || "${ctx.entityName}"`;
}
...
function renderSearchFields(ctx) {
    const fields = [...ctx.filterFields]
        .filter((field) => ['String', 'Text', 'MediumText'].includes(field.type))
        .sort((left, right) => {
