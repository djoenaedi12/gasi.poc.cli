...
function buildWebResourceContext(resource) {
    ...
    const ctx = {
        ...
        defaultColumnFields: fields.filter((field) => isDtoIncluded(field, 'summary') && isVisibleByDefault(field)),
        detailFields: fields.filter((field) => isDtoIncluded(field, 'detail')),
        filterFields: fields.filter((field) => field.filterable),
        tableFilterFields: fields.filter((field) => isUiTableFilterEnabled(field)),
        mode,
        isReadMode,
    };
    ...
    return {
        ...
        LIST_TOAST_IMPORT: isReadMode(ctx) ? '' : 'import { appToast } from "@gasi/core-ui";',
...
` : '',
        ...
        MORE_FILTER_PROPS: renderMoreFilterProps(ctx),
...
function hasExplicitSearchableFields(ctx) {
    return ctx.fields.some((field) => field.ui?.table?.searchable !== undefined);
}
...
function advancedFilterFields(ctx) {
    if (hasExplicitTableFilterConfig(ctx)) {
        return [];
    }

    return ctx.filterFields.filter((field) =>
        !isUiTableFilterEnabled(field) &&
...
function renderDetailRouterImports(ctx) {
    if (isReadMode(ctx)) {
        return 'Navigate, useParams';
    }

    return 'Navigate, useNavigate, useParams';
}
...
function renderListStoreImport(ctx) {
    return isReadMode(ctx) ? '' : 'import { useResourcePermissions } from "@gasi/core-starter";';
}
...
function renderListColumnsDeps(ctx) {
    ...
    return '[canDelete, canUpdate, handleDelete, t]';
}
...
function renderDetailActions(ctx) {
    ...
    return `actions={
                    canUpdate ? (
                        <Button
                            type="button"
                            onClick={() => navigate(${ctx.isNestedApi ? '`${listPath}/${params.id}/edit`' : `\`${ctx.routePath}/\${params.id}/edit\``})}
                        >
...
function renderListTypeImports(ctx) {
    return hasAdvancedFilters(ctx) ? 'import type { GenericFilter } from "@gasi/core-ui";' : '';
}
...
function renderListFilterImports(ctx) {
    ...
    const imports = new Set(['DataTableFilterMenu']);
    const fields = advancedFilterFields(ctx);

    if (fields.some((field) => field.type === 'ManyToOne')) {
        imports.add('LookupPicker');
    }
    ...
    return `import { ${[...imports].sort().join(', ')} } from "@gasi/core-ui";`;
}

function renderListLookupHookImports(ctx) {
    const imports = lookupRefEntities(advancedFilterFields(ctx))
        .map((ref) => `import { use${ref.pluralPascal}Page } from "../../${ref.pluralKebab}/hooks/use${ref.name}";`);
...
function renderAdvancedFilterLogic(ctx) {
    ...
    const filterPushes = fields.map((field) => {
        const stateName = advancedFilterStateName(field);
        return `        if (${stateName}) {
            filters.push({
                type: "simple",
                field: "${requestFieldName(field)}",
                operator: "${advancedFilterOperator(field)}",
                value: ${advancedFilterValue(field)},
            });
        }`;
    }).join('\n\n');

    const deps = fields.map(advancedFilterStateName).join(', ');
    const resetLines = fields
        .map((field) => `        set${_.upperFirst(advancedFilterStateName(field))}("");`)
        .join('\n');
    const countLines = fields
        .map((field) => `        ${advancedFilterStateName(field)},`)
        .join('\n');

    return `

    const advancedFilter = useMemo<GenericFilter | undefined>(() => {
...
${filterPushes}
    ...
    const advancedFilterActiveCount = [
${countLines}
...
${resetLines}
    ...
    const moreFilter = (
        ...
        >
${renderAdvancedFilterControls(ctx)}
        </DataTableFilterMenu>
    );`;
}

function renderAdvancedFilterControls(ctx) {
    return advancedFilterFields(ctx).map((field) => renderAdvancedFilterControl(field, ctx)).join('\n\n');
}

function renderAdvancedFilterControl(field, ctx) {
    const stateName = advancedFilterStateName(field);
    const setterName = `set${_.upperFirst(stateName)}`;
    const fieldLabelExpr = `t("${ctx.entityPluralKebab}.fields.${requestFieldName(field)}")`;

    if (field.type === 'ManyToOne') {
        const refPrefix = field.refEntity ? _.kebabCase(pluralize(_.lowerFirst(field.refEntity))) : '';
        const label = refPrefix ? `t("${refPrefix}.names.singular")` : fieldLabelExpr;
        return `            <div className="space-y-2">
                <label className="text-sm font-medium">{${label}}</label>
                <LookupPicker
                    title={${label}}
                    pageQuery={use${_.upperFirst(pluralize(_.lowerFirst(field.refEntity)))}Page}
                    ...
                    onChange={${setterName}}
                    onClear={() => ${setterName}("")}
                    placeholder={t("common.actions.selectEntity", { entity: ${label} })}
                    searchPlaceholder={t("common.search.byFields", { fields: ${label}.toLowerCase() })}
                    emptyMessage={t("common.empty.title", { entity: ${label} })}
                />
            </div>`;
    }
    ...
    if (field.type === 'Boolean') {
        return `            <div className="space-y-2">
                <label className="text-sm font-medium">{${fieldLabelExpr}}</label>
                <select
                    value={${stateName}}
                    onChange={(event) => ${setterName}(event.target.value)}
    ...
    return `            <div className="space-y-2">
                ...
                <Input
                    value={${stateName}}
                    onChange={(event) => ${setterName}(event.target.value)}
                    placeholder={t("common.filters.title", { entity: ${fieldLabelExpr} })}
                    type="${advancedFilterInputType(field)}"
                />
            </div>`;
}

function advancedFilterInputType(field) {
...
function renderMoreFilterProps(ctx) {
    if (!hasAdvancedFilters(ctx)) {
        return '';
    }
...
function renderDataTableFilter(field, ctx) {
    ...
    const placement = filter.placement ?? 'popover';
    const parts = [
        `                    {
                        id: "${responseName}",
                        label: ${label},
...
function inferDataTableFilterType(field) {
    if (field.type === 'Boolean' || field.type === 'Enum') return 'select';
    ...
    return allFields.map((field) => {
        ...
        if (field.type === 'ManyToOne') {
            return `                        <FormLookupPicker
                            ...
                            displayColumns={${renderLookupDisplayColumns(field)}}
                            searchFields={${renderStringArray(lookupSearchFields(field))}}
                            placeholder={t("common.actions.selectEntity", { entity: ${label.slice(1, -1)} })}
                            searchPlaceholder={t("common.search.byFields", { fields: ${label.slice(1, -1)}.toLowerCase() })}
                            emptyMessage={t("common.empty.title", { entity: ${label.slice(1, -1)} })}
                            ${props.trim()}
                        />`;
        }
...
function patchParentPageWithNestedChildTable(content, childCtx) {
    ...
    next = ensureNamedImport(next, '@gasi/core-ui', ['Button', 'Card', 'CardContent', 'CardTabs', 'CardTabsContent', 'CardTabsList', 'CardTabsTrigger', 'ServerDataTable']);
    ...
    next = insertAfterImportBlock(next, `import { use${childCtx.entityPluralPascal}Page } from "../../${childCtx.entityPluralKebab}/hooks/use${childCtx.entityName}";`);
...
function renderNestedChildTabTrigger(childCtx, marker) {
    return `                    {/* ${marker} */}
                    <CardTabsTrigger value="${childCtx.entityPluralKebab}">{{TITLE}}</CardTabsTrigger>`
...
function renderNestedChildTabsSection(childCtx, marker, generalContent) {
    return `
            ...
            <CardTabs defaultValue="general">
                <CardTabsList>
                    <CardTabsTrigger value="general">General</CardTabsTrigger>
...
function patchParentFormWithEmbeddedChild(content, childCtx, childFieldName) {
    ...
    for (const ref of lookupRefEntities(childCtx.resource.fields)) {
        const importLine = `import { use${ref.pluralPascal}Page } from "../../${ref.pluralKebab}/hooks/use${ref.name}";`;
...
function renderEmbeddedChildColumn(field) {
    ...
    if (field.type === 'ManyToOne') {
        const refName = field.refEntity;
        const refPluralPascal = _.upperFirst(pluralize(_.lowerFirst(refName)));
        return `                            {
                                name: "${requestFieldName(field)}",
                                header: "${labelForField(field)}",
                                type: "lookup",
                                pageQuery: use${refPluralPascal}Page,
                                mapOption: ${renderLookupMapOption(field)},
                                displayColumns: ${renderLookupDisplayColumns(field)},
                                searchFields: ${renderStringArray(lookupSearchFields(field))},
                            },`;
    ...
    const options = field.type === 'Enum' || type === 'select' || type === 'lookup'
        ? ', options: []'
        : '';
