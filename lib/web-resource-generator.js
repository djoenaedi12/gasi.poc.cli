...
function renderEditBreadcrumbs(ctx) {
    ...
    return `breadcrumbs={[
                    { label: t("${ctx.parentPluralKebab}.names.plural"), href: "/${ctx.parentPluralKebab}" },
                    { label: ${ctx.parentParam} ?? t("${ctx.parentPluralKebab}.names.singular"), href: parentDetailPath },
                    { label: t("${ctx.entityPluralKebab}.names.plural") },
...
function getRouteDefinitionArrayBody(content) {
    const match = content.match(/export const \w+Routes: RouteDefinition\[\] = \[([\s\S]*?)\];/);
                    ...
                    <Card>
                        <CardContent>
                            <div className="mb-4 flex items-center justify-between gap-3">
                                ...
                                <Button type="button" onClick={() => navigate(\`\${${childCtx.entityVar}ListPath}/create?backTo=\${encodeURIComponent(\`\${window.location.pathname}?tab=${childCtx.entityPluralKebab}\`)}\`)}>
                                    <Plus className="size-4" />
