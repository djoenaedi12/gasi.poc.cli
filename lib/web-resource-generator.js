...
async function generateWebResources({ webDir, resources, force = false }) {
    ...
    for (const resource of resources) {
        ...
        const resourceFiles = await collectGeneratedPaths(templateRoot, webDir, ctx, shouldInclude);
        filesByEntity[resource.entityName] = resourceFiles;
        generatedFiles.push(...resourceFiles);
        routedEntities.push(resource.entityName);
    ...
    if (routerPath) {
        ...
        for (const entityName of routedEntities) {
            filesByEntity[entityName].push(routerPath);
        }
    }
    ...
    for (const resource of resources) {
        if (resource.parent && resource.apiStyle === 'nested' && !resource.embedInParentDto) {
            addGeneratedFiles(filesByEntity, resource.parent, filterFilesForParent(webDir, parentPageFiles, resource.parent));
        }
    }
    ...
    for (const resource of resources) {
        if (resource.parent && resource.embedInParentDto) {
            addGeneratedFiles(filesByEntity, resource.parent, filterFilesForParent(webDir, parentFormFiles, resource.parent));
        }
    }

    return {
        generatedFiles: [...new Set(generatedFiles)],
        filesByEntity: Object.fromEntries(
            Object.entries(filesByEntity).map(([entityName, files]) => [entityName, [...new Set(files)]]),
        ),
    };
}

function addGeneratedFiles(filesByEntity, entityName, files) {
    if (!entityName || !files.length) {
        return;
    }

    filesByEntity[entityName] = filesByEntity[entityName] || [];
    filesByEntity[entityName].push(...files);
}

function filterFilesForParent(webDir, files, parentEntityName) {
    const parentVar = _.lowerFirst(parentEntityName);
    const parentPluralKebab = _.kebabCase(pluralize(parentVar));
    const parentFeatureDir = path.join(webDir, 'src', 'features', parentPluralKebab);
    return files.filter((file) => file.startsWith(parentFeatureDir + path.sep));
}

async function assertWebProjectRoot(webDir) {
    if (!webDir) {
        throw new Error('--plugin is required for --target web unless current directory is the web project.');
    }
...
` : '',
        PAGE_QUERY_PROP: 'pageQuery: pageQuery,',
        ...
        LIST_PAGE_QUERY: renderListPageQuery(ctx),
        LIST_HEADER_ACTIONS: renderListHeaderActions(ctx),
        LIST_HEADER_ACTIONS_SETUP: renderListHeaderActionsSetup(ctx),
        LIST_CUSTOM_TYPE: renderListCustomType(ctx),
        ...
        DETAIL_ACTIONS_SETUP: renderDetailActionsSetup(ctx),
...
function renderListReactImport(ctx) {
    const imports = new Set(['useCallback', 'useMemo']);
...
function renderListColumnsDeps(ctx) {
    if (isReadMode(ctx)) {
        return ctx.isNestedApi ? '[custom, listPath, t]' : '[custom, t]';
    }

    return ctx.isNestedApi ? '[canDelete, canUpdate, custom, handleDelete, listPath, t]' : '[canDelete, canUpdate, custom, handleDelete, t]';
}

function renderListDefaultHeaderActions(ctx) {
    if (isReadMode(ctx)) return '';

    return `canCreate || canUpload ? (
                        <>
                            {canUpload ? (
...
function renderListHeaderActions(ctx) {
    const defaultActions = renderListDefaultHeaderActions(ctx);
    if (!defaultActions) return 'actions={headerActions}';
    return 'actions={headerActions}';
}

function renderListHeaderActionsSetup(ctx) {
    const defaultActions = renderListDefaultHeaderActions(ctx) || 'null';
    return `
    const defaultHeaderActions = ${defaultActions};
    const headerActions = custom.list?.headerActions?.(defaultHeaderActions) ?? defaultHeaderActions;
`;
}

function renderListPageQuery(ctx) {
    const request = 'custom.list?.beforeSearch?.(request) ?? request';

    if (ctx.isNestedApi) {
        return `
    const pageQuery = useCallback(
        (request: SearchRequest) => use${ctx.entityPluralPascal}Page(${ctx.parentParam} ?? "", ${request}),
        [${ctx.parentParam}, custom],
    );
`;
    }
    ...
    return `
    const pageQuery = useCallback(
        (request: SearchRequest) => use${ctx.entityPluralPascal}Page(${request}),
        [custom],
    );
`;
}

function renderListCustomType(ctx) {
    return `type ${ctx.entityName}ListCustom = {
    list?: {
        beforeSearch?: (request: SearchRequest) => SearchRequest;
        columns?: (columns: ColumnDef<${ctx.entityName}Summary>[]) => ColumnDef<${ctx.entityName}Summary>[];
...
function renderDetailDefaultActions(ctx) {
    if (isReadMode(ctx)) return '';
...
function renderDetailActionsSetup(ctx) {
    const defaultActions = renderDetailDefaultActions(ctx) || 'null';
    return `    const defaultHeaderActions = ${defaultActions};
    const headerActions = custom.detail?.headerActions?.(defaultHeaderActions, {
        data: ${ctx.entityVar},
...
import type { ${ctx.entityName}Summary } from "../types/${ctx.entityVar}.types";`;
}
...
async function checkTemplateConflicts(templateRoot, targetRoot, ctx, shouldInclude) {
    ...
    for (const entry of entries) {
        ...
        if (await fs.pathExists(targetPath)) {
            throw new Error(`File already exists: ${targetPath}. Run resource plan first, then resource sync to update generated files.`);
        }
    }
}
...
function renderDetailFields(ctx) {
    const screen = detailScreenConfig(ctx);

    if (screen) {
        return renderDetailScreen(ctx, screen);
    }

    return renderDetailScreen(ctx, {
        tabs: [{
            id: 'general',
            title: 'General',
            sections: [{ rows: fieldsToLayoutRows(ctx.detailFields) }],
        }],
    });
}

function renderFormFields(ctx) {
    const createScreen = createScreenConfig(ctx);
    const editScreen = editScreenConfig(ctx);

    if (createScreen || editScreen) {
        const createFields = uniqueByName(ctx.createFields);
        const editFields = uniqueByName(ctx.updateFields);
        const createMarkup = renderFormScreen(ctx, createFields, createScreen ?? defaultFormScreen(createFields), 12);
        const editMarkup = renderFormScreen(ctx, editFields, editScreen ?? createScreen ?? defaultFormScreen(editFields), 12);
...
${editMarkup}
    ...
    return renderFormScreen(ctx, uniqueByName([...ctx.createFields, ...ctx.updateFields]), null, 12);
}

function createScreenConfig(ctx) {
    return normalizeScreenConfig(ctx.resource.ui?.create) ?? normalizeScreenConfig(ctx.resource.ui?.form);
}

function editScreenConfig(ctx) {
    return normalizeScreenConfig(ctx.resource.ui?.edit) ??
        normalizeScreenConfig(ctx.resource.ui?.form) ??
        normalizeScreenConfig(ctx.resource.ui?.create);
}

function detailScreenConfig(ctx) {
    return normalizeScreenConfig(ctx.resource.ui?.detail) ??
        normalizeScreenConfig(ctx.resource.ui?.edit) ??
        normalizeScreenConfig(ctx.resource.ui?.form) ??
        normalizeScreenConfig(ctx.resource.ui?.create);
}

function normalizeScreenConfig(screen) {
    if (!screen) {
        return null;
    }
...
function defaultFormScreen(fields) {
    return {
        tabs: [{
            id: 'general',
            title: 'General',
            sections: [{ rows: fieldsToLayoutRows(fields) }],
        }],
    };
}

function fieldsToLayoutRows(fields) {
    return fields.map((field) => ({
        columns: [{ field: requestFieldName(field), span: 1 }],
    }));
}

function formLayoutConfig(ctx) {
    const form = ctx.resource.ui?.form;

    if (form?.sections?.length) {
        return { sections: form.sections };
    }

    if (form?.rows?.length) {
        return { sections: [{ rows: form.rows }] };
    }

    return null;
}

function detailLayoutConfig(ctx) {
    const detail = ctx.resource.ui?.detail;
    const form = ctx.resource.ui?.form;
    ...
    if (detail?.rows?.length) {
        return { sections: [{ rows: detail.rows }] };
    }
...
function renderFormLayout(ctx, fields, layout) {
    const fieldMap = layoutFieldMap(fields);

    return `                    <div className="space-y-6">
${layout.sections.map((section) => renderFormSection(ctx, fieldMap, section)).join('\n\n')}
                    </div>`;
}

function renderFormScreen(ctx, fields, screen, indentSize) {
    const normalized = screen ?? defaultFormScreen(fields);
    const tabs = normalized.tabs.filter((tab) => tab.sections?.length);
    const fieldMap = layoutFieldMap(fields);
    const indent = ' '.repeat(indentSize);
    const defaultTab = tabs[0]?.id ?? 'general';

    if (tabs.length <= 1) {
        return renderFormTabCard(ctx, fieldMap, tabs[0] ?? defaultFormScreen(fields).tabs[0], indentSize);
    }

    return `${indent}<CardTabs defaultValue="${escapeTsString(defaultTab)}" className="w-full">
${indent}    <CardTabsList>
${tabs.map((tab) => `${indent}        <CardTabsTrigger value="${escapeTsString(tab.id)}">${escapeJsxText(tab.title || _.startCase(tab.id))}</CardTabsTrigger>`).join('\n')}
${indent}    </CardTabsList>

${tabs.map((tab) => `${indent}    <CardTabsContent value="${escapeTsString(tab.id)}">
${renderFormTabCard(ctx, fieldMap, tab, indentSize + 8)}
${indent}    </CardTabsContent>`).join('\n\n')}
${indent}</CardTabs>`;
}

function renderFormTabCard(ctx, fieldMap, tab, indentSize) {
    const indent = ' '.repeat(indentSize);
    const title = tab?.title || 'General';
    const sections = tab?.sections?.length ? tab.sections : [];

    return `${indent}<Card>
${indent}    <CardHeader>
${indent}        <CardTitle>${escapeJsxText(title)}</CardTitle>
${indent}        <CardDescription>
${indent}            {t("common.descriptions.generalEntityInfo", { entity: t("${ctx.entityPluralKebab}.names.singular") })}
${indent}        </CardDescription>
${indent}    </CardHeader>

${indent}    <CardContent>
${renderFormSections(ctx, fieldMap, sections, indentSize + 8)}
${indent}    </CardContent>
${indent}</Card>`;
}

function renderDetailScreen(ctx, screen) {
    const fieldMap = layoutFieldMap(ctx.detailFields);
    const tabs = screen.tabs.filter((tab) => tab.sections?.length);
    const defaultTab = tabs[0]?.id ?? 'general';

    return `            <CardTabs defaultValue={new URLSearchParams(window.location.search).get("tab") || "${escapeTsString(defaultTab)}"} className="w-full max-w-5xl">
                <CardTabsList>
${tabs.map((tab) => `                    <CardTabsTrigger value="${escapeTsString(tab.id)}">${escapeJsxText(tab.title || _.startCase(tab.id))}</CardTabsTrigger>`).join('\n')}
                </CardTabsList>

${tabs.map((tab) => `                <CardTabsContent value="${escapeTsString(tab.id)}">
${renderDetailTabCard(ctx, fieldMap, tab)}
                </CardTabsContent>`).join('\n\n')}
            </CardTabs>`;
}

function renderDetailTabCard(ctx, fieldMap, tab) {
    const title = tab?.title || 'General';
    const sections = tab?.sections?.length ? tab.sections : [];

    return `                    <Card>
                        <CardHeader>
                            <CardTitle>${escapeJsxText(title)}</CardTitle>
                            <CardDescription>
                        ...
                        <CardContent>
${renderDetailSections(ctx, fieldMap, sections)}
                        </CardContent>
                    </Card>`;
}

function renderFormSections(ctx, fieldMap, sections, indentSize) {
    const indent = ' '.repeat(indentSize);

    if (!sections.length) {
        return `${indent}<div className="grid gap-5" />`;
    }

    return `${indent}<div className="space-y-6">
${sections.map((section) => renderFormSection(ctx, fieldMap, section, indentSize + 4)).join('\n\n')}
${indent}</div>`;
}

function renderDetailSections(ctx, fieldMap, sections) {
    if (!sections.length) {
        return `                            <dl className="grid gap-5" />`;
    }

    return `                            <div className="space-y-6">
${sections.map((section) => renderDetailSection(ctx, fieldMap, section)).join('\n\n')}
                            </div>`;
}

function renderFormSection(ctx, fieldMap, section, indentSize = 24) {
    const indent = ' '.repeat(indentSize);

    return `${indent}<section className="space-y-4">
${renderLayoutSectionHeader(section, indentSize + 4)}
${indent}    <div className="space-y-5">
${(section.rows ?? []).map((row) => renderFormRow(ctx, fieldMap, row, indentSize + 8)).join('\n\n')}
${indent}    </div>
${indent}</section>`;
}

function renderDetailSection(ctx, fieldMap, section) {
    return `                                <section className="space-y-4">
${renderLayoutSectionHeader(section, 36)}
                                    <div className="space-y-5">
${(section.rows ?? []).map((row) => renderDetailRow(ctx, fieldMap, row)).join('\n\n')}
                                    </div>
                                </section>`;
}

function renderLayoutSectionHeader(section, indentSize) {
    const lines = [];
    const indent = ' '.repeat(indentSize);

    if (section.title) {
        lines.push(`${indent}    <h3 className="text-sm font-semibold text-foreground">${escapeJsxText(section.title)}</h3>`);
    }

    if (section.description) {
        lines.push(`${indent}    <p className="text-sm text-muted-foreground">${escapeJsxText(section.description)}</p>`);
    }
...
function renderFormRow(ctx, fieldMap, row, indentSize = 32) {
    const columns = normalizeLayoutColumns(row);
    const indent = ' '.repeat(indentSize);

    return `${indent}<div className="${layoutGridClass(columns)}">
${columns.map((column) => renderFormField(ctx, layoutField(ctx, fieldMap, column.field), indentSize + 4)).join('\n\n')}
${indent}</div>`;
}

function renderDetailRow(ctx, fieldMap, row) {
    const columns = normalizeLayoutColumns(row);

    return `                                        <dl className="${layoutGridClass(columns)}">
${columns.map((column) => renderDetailField(ctx, layoutField(ctx, fieldMap, column.field), 44)).join('\n\n')}
                                        </dl>`;
}

function normalizeLayoutColumns(row) {
    return (row.columns ?? [])
...
function normalizeLayoutSpan(span) {
    const value = Number(span ?? 1);
    return Number.isFinite(value) && value > 0 ? Math.round(value) : 1;
}

function layoutGridClass(columns) {
    if (columns.length <= 1) {
        return 'grid gap-5';
    }

    const template = columns.map((column) => `${column.span}fr`).join('_');
    return `grid grid-cols-1 gap-5 md:[grid-template-columns:${template}]`;
}

function layoutFieldMap(fields) {
    const map = new Map();

    for (const field of fields) {
        map.set(field.name, field);
        map.set(requestFieldName(field), field);
        map.set(responseFieldName(field), field);
    }

    return map;
}

function layoutField(ctx, fieldMap, fieldName) {
    const field = fieldMap.get(fieldName);

    if (!field) {
        throw new Error(`${ctx.resource.entityName}.ui layout references unknown or unavailable field "${fieldName}".`);
    }

    return field;
}

function renderDetailField(ctx, field, indentSize) {
    const indent = ' '.repeat(indentSize);

    return `${indent}<div>
${indent}    <dt className="text-sm font-medium text-muted-foreground">{t("${ctx.entityPluralKebab}.fields.${requestFieldName(field)}")}</dt>
${indent}    <dd className="mt-1 text-sm">{String(${ctx.entityVar}.${responseFieldName(field)} ?? t("common.fallbackValue"))}</dd>
${indent}</div>`;
}

function renderFormField(ctx, field, indentSize) {
    const fieldName = requestFieldName(field);
    ...
    if (field.type === 'Boolean') {
        return `${indent}<FormSwitch form={form} name="${fieldName}" label=${label}${props} />`;
    }

    if (field.type === 'Date') {
        return `${indent}<FormDatePicker form={form} name="${fieldName}" label=${label}${props} />`;
    }

    if (field.type === 'DateTime' || field.type === 'Instant') {
        return `${indent}<FormDateTimePicker form={form} name="${fieldName}" label=${label}${props} />`;
    }

    if (field.type === 'ManyToOne') {
        const entityLabel = lookupEntityLabelExpression(field, label.slice(1, -1));
        return `${indent}<FormLookupPicker
${indent}    form={form}
${indent}    name="${fieldName}"
${indent}    label=${label}
${indent}    lookup={${renderLookupConfigExpression(field)}}
${indent}    placeholder={t("common.actions.selectEntity", { entity: ${entityLabel} })}
${indent}    searchPlaceholder={t("common.search.byFields", { fields: ${entityLabel}.toLowerCase() })}
${indent}    emptyMessage={t("common.empty.title", { entity: ${entityLabel} })}
${indent}    ${props.trim()}
${indent}/>`;
    }

    if (field.type === 'Enum') {
        return `${indent}<FormSelect form={form} name="${fieldName}" label=${label} options={${renderEnumOptions(field)}}${props} />`;
    }

    if (field.type === 'Text' || field.type === 'MediumText') {
        return `${indent}<FormTextarea form={form} name="${fieldName}" label=${label} rows={4} textareaClassName="min-h-28"${props} />`;
    }

    return `${indent}<FormInput form={form} name="${fieldName}" label=${label}${inputTypeAttr(field)}${props} />`;
}
...
function collectFormImports(ctx) {
    ...
    if (fieldTabCount(createScreenConfig(ctx)) > 1 || fieldTabCount(editScreenConfig(ctx)) > 1) {
        imports.add('CardTabs');
        imports.add('CardTabsContent');
        imports.add('CardTabsList');
        imports.add('CardTabsTrigger');
    }
...
function fieldTabCount(screen) {
    return screen?.tabs?.filter((tab) => tab.sections?.length).length ?? 0;
}
...
function insertIntoRouteDefinitionArray(content, spreadLines) {
    const markerPattern = /(\n\s*\/\/ routes akan ditambahkan oleh `gasi resource (?:create|sync)`)/;
